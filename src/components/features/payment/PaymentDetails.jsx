...
import { useGetPaymentByIdQuery, useUpdatePaymentMutation } from "../../../services/paymentApi.js";

const PaymentDetails = ({ paymentId, onClose, refetchPayments, onNotify }) => {
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            ...
            onNotify("success", response.message || "Payment updated successfully!");
            refetchPayments();
        } catch (err) {
            console.error(err);
            onNotify("error", err.data?.message || "Failed to update payment.");
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-40 z-[1100] flex justify-center items-start pt-10 overflow-auto">
            <div className="bg-white rounded-lg shadow-md max-w-5xl w-full p-6 relative z-[1110]">
                <button
                    onClick={onClose}
                ...
                <h2 className="text-2xl font-semibold text-gray-800 mb-1">Payment #{payment.id}</h2>

                <p className="text-sm text-gray-500 mb-4">
                    User: <span className="font-medium text-gray-700">{payment.user?.username || "N/A"}</span> | Paid At:{" "}
                    <span className="font-medium text-gray-700">{new Date(payment.paidAt).toLocaleString()}</span>
                </p>
