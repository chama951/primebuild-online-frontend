        ...
        {key: "userId", header: "ID", render: (item) => <div className="text-sm text-gray-500">#{item.userId}</div>},
        {
            key: "username", header: "Username", render: (item) => (
                <div className="text-sm font-medium">
                    {item.username}
                    {!item.enabled &&
                        <span className="ml-2 px-2 py-0.5 text-xs bg-red-100 text-red-800 rounded">Disabled</span>}
                </div>
            )
        },
        {
            key: "role", header: "Role", render: (item) => (
                <div><span
                    className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">{item.role?.roleName || 'N/A'}</span>
                </div>
            )
        },
    ];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex gap-4">
                            <div className="relative flex-1">
                                ...
                                {searchTerm && <button onClick={() => setSearchTerm("")}
                                                       className="absolute right-3 top-2.5 text-gray-400">✕</button>}
                            </div>
                            <select className="w-40 p-2 border rounded text-sm" value={filterType}
                    ...
                    <DataTable items={paginatedUsers} selectedItem={selectedUser} onSelectItem={handleSelectUser}
                               onDeleteItemClick={handleDeleteUser} isLoading={usersLoading} columns={columns}
                ...
                <div className="space-y-4">
                    <div className="bg-white rounded-lg border p-4">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <select className="w-full p-2 border rounded" value={roleId}
                            ...
                            {selectedUser && (
                                <div className="border rounded p-3 space-y-2 bg-gray-50">
                                    <div className="flex justify-between text-sm"><span className="text-gray-600">User ID:</span><span
                                        className="font-medium">#{selectedUser.userId}</span></div>
                                    <div className="flex justify-between text-sm"><span
                                        className="text-gray-600">Email:</span><span
                                        className="font-medium">{selectedUser.email}</span></div>
                                    ...
                                    <div className="flex justify-between text-sm"><span
                                        className="text-gray-600">Type:</span><span
                                        className="font-medium">{userType}</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-gray-600">Sign Up:</span><span
                                        className="font-medium">{selectedUser.signUpMethod}</span></div>
                                    <div className="flex justify-between text-sm"><span
                                        className="text-gray-600">Created:</span><span
                                        className="font-medium">{selectedUser.createdDate}</span></div>
                                </div>
                            )}
                            {selectedUser && (
                                <div className="flex gap-2 pt-2">
                                    <button type="submit"
                                            disabled={isSubmitting || !username.trim() || !email.trim() || !roleId}
                    ...
                    <div className="bg-white rounded-lg border p-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm"><span
                                className="text-gray-600">Total Users:</span><span
                                className="font-medium">{users.length}</span></div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-600">Staff:</span>
                                <span
                                    className="font-medium">{users.filter(u => u.role?.roleName?.toLowerCase().includes('admin') || u.role?.roleName?.toLowerCase().includes('staff')).length}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            <div className="flex justify-between text-sm"><span
                                className="text-gray-600">Enabled:</span><span
                                className="font-medium text-green-600">{users.filter(u => u.enabled).length}</span>
                            </div>
                            <div className="flex justify-between text-sm"><span
                                className="text-gray-600">Disabled:</span><span
                                className="font-medium text-red-600">{users.filter(u => !u.enabled).length}</span></div>
                        </div>
                    </div>
                </div>
            </div>
