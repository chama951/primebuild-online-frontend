...
import {useUpdateInvoiceMutation} from "../../../services/invoiceApi.js";
...
const InvoiceDetails = ({invoice, onClose, isSubmitting}) => {
    ...
    return (
        <>
            <div
                className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-center items-start pt-10 overflow-auto">
                  ...
                  {invoiceStatus}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {invoice.invoiceItems.map((invoiceItem) => {
                            const discountedUnit = invoiceItem.unitPrice - invoiceItem.discountPerUnite;
                            ...
                            return (
                                ...
                                >
                                    <div className="text-xs font-semibold text-gray-500 uppercase mb-1">
                                        {invoiceItem.item.component?.componentName || "N/A"}
                                    </div>
                      ...
                      {invoiceItem.item.manufacturer?.manufacturerName || "-"}
                                    ...
                                    <p className="text-gray-600 text-sm">
                                        Qty: <span className="font-medium">{invoiceItem.invoiceQuantity}</span>
                                    </p>

                                    <p className="text-gray-600 text-sm">
                                        Unit Price: <span
                                        className="font-medium">Rs {formatCurrency(invoiceItem.unitPrice)}</span>
                                    </p>

                                    {invoiceItem.discountPerUnite > 0 && (
                                        <p className="text-green-600 text-sm">
                                            Discounted: Rs {formatCurrency(discountedUnit)}
                                        </p>
                                    )}

                                    <div className="mt-2 text-sm font-semibold text-gray-800">
                                        Subtotal: Rs {formatCurrency(invoiceItem.subtotal)}
                                    </div>
                                </div>
                            );
                        })}
